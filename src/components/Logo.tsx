...
/**
 * New England CareFlow mark — the official corporate artwork, same paths as
 * the Visual Notes site.
 *
 * A stethoscope drawn so its binaural and tubing form the "N": navy stem and
 * shoulder, seagreen tubing sweeping into the chest piece. Fills come from
 * CSS custom properties so the artwork itself is never edited per background.
 */
export function Mark({
  ...
  className?: string;
  title?: string;
  /** Sweep a specular band across the artwork, timed to the revolve. */
  glare?: boolean;
}) {
...
/** The mark's height as a fraction of its width (from the viewBox). */
const ASPECT = 136.6 / 221.2;

/**
 * The Visual Notes revolve on the flat artwork: the mark rests, flips a fast
 * 360° on its Y axis like a coin, then catches a glare sweep as it settles.
 * One 7s cycle, CSS only.
 */
...
/** Full lockup: spinning mark + company name, as in the Visual Notes header. */
export function Wordmark({ className = "" }: { className?: string }) {
  return (
    <span className={`flex items-center gap-3 ${className}`}>
      <SpinningMark width={40} />
      <span className="font-display text-[0.94rem] leading-none font-extrabold tracking-[0.045em] uppercase">
